...
export class UserController {
	...
	static fpReset(req: Request, res: Response) {
		const email = req.body.email;
		const otpcode = Nodemailer.otpgenerator();
		User.findOneAndUpdate({ email: email }, { code: otpcode }, { new: true })
			.then(data => {
				if (!!data) {
					Nodemailer.sendEmail("jaiswalmayank450@gmail.com", req.body.email, "verification-mail-is-here", otpcode);
					res.status(200).send(data);
	...
	static fpSet(req: Request, res: Response) {
		const newotp = req.body.otpcheck;
		const newpassword = req.body.newpassword;
		const emailtobereset = req.body.emailtobereset;
		User.findOne({ email: emailtobereset }).then(data => {
			if (data.code === newotp) {
				User.findOneAndUpdate({ email: data.email }, { password: newpassword }, { new: true }).then(data => {
					if (!!data) {
						res.status(200).send(data);
					}
				}).catch((err) => {
